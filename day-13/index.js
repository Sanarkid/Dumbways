...
// API untuk edit proyek
app.get("/api/projects/:id", async (req, res) => {
  const { id } = req.params;

  try {
    const result = await pool.query("SELECT * FROM my_projects WHERE id = $1", [
      id,
    ]);
  ...
  try {
    const result = await pool.query(
      `UPDATE my_projects
       ...
       WHERE id=$7
       RETURNING *`,
      [title, startDate, endDate, language, description, imageUrl, id]
    );
